...
#ATEV <- readRDS("results/ATEV_prenormalization.RDS")
#genes_in_data <- Features(ATEV)
...
ATEV <- readRDS("results/ATEV_post_annotation.RDS")
DefaultAssay(ATEV) <- "RNA"
diff_markers <- FindAllMarkers(object = ATEV, densify = TRUE)
...
GO_analysis <- function(cell_type) {
  ...
  write.csv(UP, file = paste0(output_dir, "/GO_UP_ATEV_", cell_type, ".csv"), row.names = F)
  write.csv(DOWN, file = paste0(output_dir, "/GO_DOWN_ATEV_", cell_type, ".csv"), row.names = F)
